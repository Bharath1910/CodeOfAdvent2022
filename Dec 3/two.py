...
"""
priorityOrder = list("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
...
for i in range(len(seperatedTestCases)):
    if seperatedTestCases[i] == "":
        pass

    else:
        temp.append(seperatedTestCases[i])
        a += 1

        if a == 3:
            seperatedGroups.append(temp)
            temp = []
            a = 0

for i in seperatedGroups:
    setOne, setTwo, setThree = set(i[0]), set(i[1]), set(i[2])
    repeatedCharacters = setOne.intersection(setTwo).intersection(setThree)

    for i in repeatedCharacters:
        score += priorityOrder.index(i) + 1
